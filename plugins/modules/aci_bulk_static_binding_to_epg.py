...
options:
  ...
  epgs:
    description:
    - List of epg configurations, elements in the form of a dictionary.
    - Module level epg attributes are mutual exclusive to this epgs list.
...
def check_and_get_filtered_bindings(
    ...
    if interface_configs is not None:
        for interface_config in interface_configs:
            static_path = check_and_get_static_path(
                aci, interface_config, module_interface_type
            )
            static_paths_dn.append(f"{epg_dn}/rspathAtt-[{static_path}]")

    existing_bindings_object = {}
    for epg in existing_epgs:
        epg_children = epg.get("fvAEPg", {}).get("children", [])
        for binding in deepcopy(epg_children):
            ...
            # else:
            #    # Todo: To be discussed if we should filter out the bindings? This would break the current behaviour
            #    epg_children.remove(binding)
    return existing_bindings_object


def check_and_get_static_path(
    aci: ACIModule, interface_config: dict, module_interface_type: str = None
) -> str:
    pod_id = interface_config.get("pod_id")
    interface = interface_config.get("interface")
    extpaths = interface_config.get("extpaths")
    interface_type = (
        interface_config.get("interface_type") or module_interface_type or "switch_port"
    )
    if interface_type in ["fex", "fex_vpc", "fex_port_channel"] and extpaths is None:
        aci.fail_json(
            msg="extpaths is required when interface_type is: {0}".format(
                interface_type
            )
        )
    leafs = check_and_get_leafs(aci, interface_config.get("leafs"), interface_type)
    extpaths = check_and_get_ext_paths(
        aci=aci,
...
def get_existing_epg_based(
    ...
    aci.get_existing()
    if interface_configs is not None:
        return check_and_get_filtered_bindings(
...
def get_existing_epgs_based(
    aci: ACIModule, epgs: list[dict], interface_configs: list[dict]
) -> tuple[dict, list]:
    BATCH_SIZE = 6

    epg_list_existing = get_api_call(
        aci=aci, uri="/api/class/fvAEPg.json?rsp-prop-include=config-only"
    )

    epg_dict_existing = dict()
    for epg in epg_list_existing:
        epg_dict_existing[epg.get("fvAEPg").get("attributes").get("dn")] = epg

    epg_dict_existing_filtered = dict()
    for epg in epgs:
        dn = f"uni/tn-{epg.get('tenant')}/ap-{epg.get('ap')}/epg-{epg.get('epg')}"
        if epg := epg_dict_existing.get(dn):
            epg_dict_existing_filtered[dn] = epg
        else:
            aci.fail_json(
                msg=f"The EPG with DN {dn} is not present in the Fabric. Can't proceed!"
            )
    ...
    if interface_configs is not None:
        for interface_config in interface_configs:
            static_paths.append(
                check_and_get_static_path(aci=aci, interface_config=interface_config)
            )

    existing_bindings = []
    uri = "/api/class/fvRsPathAtt.json?rsp-prop-include=config-only"
    aci.url = f"{aci.base_url}{uri}"
    if not aci.suppress_previous:
        if len(static_paths) == 0:
            existing_bindings = get_api_call(aci=aci, uri=uri)
        elif len(static_paths) == 1:
            filter_string = (
                f'query-target-filter=eq(fvRsPathAtt.tDn,"{static_paths[0]}")'
            )
            existing_bindings = get_api_call(aci=aci, uri=f"{uri}&{filter_string}")
        else:
            index = 0
            while index < len(static_paths):
                batch = static_paths[index : index + BATCH_SIZE]
                joined_string = ",".join(
                    [f'eq(fvRsPathAtt.tDn,"{path}")' for path in batch]
                )
                filter_string = f"query-target-filter=or({joined_string})"

                existing_bindings.extend(
                    get_api_call(aci=aci, uri=f"{uri}&{filter_string}")
                )
                index += BATCH_SIZE
    ...
    for binding in existing_bindings:
        ...
        if binding_attributes.get("tDn") not in static_paths and len(static_paths) > 0:
            # Ensure we only Proceed with Bindings which are in the interface_configs.
            # Should already be the case according to the query.
            continue
        binding_dn = binding_attributes.get("dn")
        epg_dn = binding_dn.split("/rspathAtt-")[0]
        if epg_existing := epg_dict_existing_filtered.get(epg_dn):
            ...
            continue
    return existing_binding_objects, list(epg_dict_existing_filtered.values())


def get_api_call(aci: ACIModule, uri: str):
    ...
    return analyze_aci_get_response(aci, info, response)


def analyze_aci_get_response(
    aci: ACIModule, info: dict, resp_objects
) -> Union[list, dict]:
    ...
    response_data = json.loads(resp_objects.read())
    return response_data["imdata"]


def should_post_binding(
    binding_to_push: dict, existing_binding: dict, state: str
) -> bool:
    should_push = False
    if state == "present" and existing_binding is None:
        should_push = True
    elif state == "present" and existing_binding is not None:
        existing_attributes = existing_binding.get("fvRsPathAtt", {}).get(
            "attributes", {}
        )
        new_attributes = binding_to_push.get("fvRsPathAtt", {}).get("attributes", {})
...
def check_and_get_ext_paths(
    aci: ACIModule, extpaths_config: list, interface_type: str
) -> Optional[str]:
...
def check_and_get_leafs(aci: ACIModule, leafs_config: list, interface_type: str) -> str:
...
def check_and_get_primary_encap_id(aci: ACIModule, primary_encap_id: str) -> int:
    if primary_encap_id is not None:
        try:
            ...
            if isinstance(primary_encap_id, str) and primary_encap_id != "unknown":
                aci.fail_json(
                    msg="Valid VLAN assignments are from 1 to 4096 or unknown. %s" % e
                )
...
def merge_binding_change_to_existing_config_dict(
    ...
    tenant_existing = next(
        (
            tenant_exist
            for tenant_exist in config_children
            if tenant_exist.get("fvTenant", {}).get("attributes", {}).get("name", None)
            == tenant
        ),
        None,
    )
    if tenant_existing is not None:
        ...
        ap_existing = next(
            (
                ap_exist
                for ap_exist in tenant_children
                if ap_exist.get("fvAp", {}).get("attributes", {}).get("name", None)
                == ap
            ),
            None,
        )
        ...
        if ap_existing is not None:
            ...
            epg_existing = next(
                (
                    epg_exist
                    for epg_exist in fv_ap_children
                    if epg_exist.get("fvAEPg", {})
                    .get("attributes", {})
                    .get("name", None)
                    == epg
                ),
        ...
        config_children.append(binding_tree_to_push)


def get_epgs_exit_values(
    aci: ACIModule, changed: bool, check_mode: bool, epgs, interface_configs
):
    result = aci.result
    ...
    if aci.suppress_verification:
        ...
        existing_binding_objects, result["current"] = get_existing_epgs_based(
            aci=aci, epgs=epgs, interface_configs=interface_configs
        )
    elif changed is False:
        result["current"] = result["previous"]


def check_and_get_encap_id(aci: ACIModule, encap_id: int):
...
def main():
    ...
    argument_spec.update(
        ...
        interface_configs=dict(
            ...
            options=dict(
                description=dict(type="str", aliases=["descr"]),
                encap_id=dict(type="int", aliases=["vlan", "vlan_id"]),
                primary_encap_id=dict(
                    type="str", aliases=["primary_vlan", "primary_vlan_id"]
                ),
                deploy_immediacy=dict(type="str", choices=["immediate", "lazy"]),
                interface_mode=dict(
                    type="str",
    ...
    if is_single_epg is True:
        # Case single EPG provided
    ...
    elif epgs is not None and len(epgs) > 0:
        # Case multiple EPG provided
        existing_binding_objects, aci.existing = get_existing_epgs_based(
            aci=aci, epgs=epgs, interface_configs=interface_configs
        )
        static_bindings = epgs
    elif state == "query" and (epg is None or not epgs):
        # Case no EPG nor EPGs provided. Getting all Bindings.
        existing_binding_objects = get_existing_epg_based(
            aci=aci,
            tenant=tenant,
            ap=ap,
    ...
    if state == "present" or state == "absent":
        ...
        for binding in static_bindings:
            ...
            epg_children_to_push = list()
            if is_single_epg is True:
                epg_to_push = dict(
                    fvAEPg=dict(
                        attributes=dict(name=epg_inner), children=epg_children_to_push
                    )
                )
            else:
                # Ensure Proposed gets the dn attribute and the Config dict not. Use the same children List as
                # both needs the Binding Changes
                epg_to_push = dict(
                    fvAEPg=dict(
                        attributes=dict(name=epg_inner), children=epg_children_to_push
                    )
                )
                proposed_epgs.append(
                    dict(
                        fvAEPg=dict(
                            attributes=dict(
                            ...
                            children=epg_children_to_push,
                        )
                    )
                )
            last_epg_reference = epg_to_push
            ap_to_push = dict(
                fvAp=dict(attributes=dict(name=ap_inner), children=[epg_to_push])
            )
            binding_tree_to_push = dict(
                fvTenant=dict(attributes=dict(name=tenant_inner), children=[ap_to_push])
            )
            for interface_config in interface_configs:
                pod_id = interface_config.get("pod_id")
                interface = interface_config.get("interface")
                ...
                description = (
                    interface_config.get("description") or module_description_inner
                )
                deploy_immediacy = (
                    interface_config.get("deploy_immediacy")
                    or module_deploy_immediacy_inner
                )
                interface_type = (
                    interface_config.get("interface_type")
                    or module_interface_type_inner
                    or "switch_port"
                )
                encap_id = interface_config.get("encap_id") or module_encap_id_inner
                primary_encap_id = (
                    interface_config.get("primary_encap_id")
                    or module_primary_encap_id_inner
                )

                interface_mode = (
                    interface_config.get("interface_mode")
                    or module_interface_mode_inner
                )

                leafs = check_and_get_leafs(
                    aci, interface_config.get("leafs"), interface_type
                )

                extpaths = check_and_get_ext_paths(
                    aci=aci, extpaths_config=extpaths, interface_type=interface_type
                )

                encap_id = check_and_get_encap_id(aci=aci, encap_id=encap_id)

                primary_encap_id = check_and_get_primary_encap_id(aci, primary_encap_id)
                ...
                existing_binding = existing_binding_objects.get(
                    f"uni/tn-{tenant_inner}/ap-{ap_inner}/epg-{epg_inner}/rspathAtt-[{static_path}]"
                )
                ...
                binding_to_push = dict(fvRsPathAtt=dict(attributes=attributes))

                if (
                    should_post_binding(
                        binding_to_push=binding_to_push,
                        existing_binding=existing_binding,
                        state=state,
                    )
                    is True
                ):
                    epg_to_push.get("fvAEPg", {}).get("children").append(
                        binding_to_push
                    )
                    merge_binding_change_to_existing_config_dict(
                        ...
                        ap=ap_inner,
                        epg=epg_inner,
                        ap_to_push=ap_to_push,
                        epg_to_push=epg_to_push,
                        binding_tree_to_push=binding_tree_to_push,
                    )
        ...
        if is_single_epg is True:
            aci.payload(
                aci_class="fvAEPg",
                class_config=dict(),
                child_configs=last_epg_reference.get("fvAEPg", {}).get("children", []),
            )
            aci.get_diff(aci_class="fvAEPg")
            aci.post_config()
            aci.exit_json()
        else:
            aci.proposed = proposed_epgs
